...
def seqrange_output(cnt,test_cnt,save):
    ...
    if save ==0:
        ...
        if seq_jdge_array[4].get() ==1: #and seq_runopt < 2 
            fname = now_fname.strftime('%y%m%d_%H%M%S_') + entrypi2_3.get().replace('file name', '') + seq_name.get() + '_op'
        ...
        if len(fld_list) > 1:
            ...
            path = os.getcwd()+'/Output/'
            df_all_file = now_fname.strftime('%y%m%d_%H%M%S_') + entrypi2_3.get().replace('file name', '') + '_OutputAll'
...
def seqrange_run():
    ...
    if float(piset_value_array[0][2]) == 0 or float(piset_value_array[1][2])==0 or float(piset_value_array[2][2]) == 0:
...
def train_conv():#パルス列を記号に変換
    for y, row in enumerate(pulse_train_array, 0):
        for x, col in enumerate(row):
            ...
            if aph1 ==0b01:     aph = "RP"
            elif aph1 ==0b10:   aph = "NP"
            else :              aph = "--"
            if aph2 ==0b01:     aph = "rd"
            elif aph2 ==0b10:   aph = "nd"
            elif aph2 ==0b11:   aph = "wd"

            if bph1 ==0b01:   bph = "RP"
            elif bph1 ==0b10:   bph = "NP"
            else :              bph = "--"
            if bph2 ==0b01:     bph = "rd"
            elif bph2 ==0b10:   bph = "nd"
            elif bph2 ==0b11:   bph = "wd"
